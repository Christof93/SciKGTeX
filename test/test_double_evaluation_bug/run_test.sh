#!/bin/bash
printf "\n---------------------------- Executing double evaluation test -----------------------------------.\n" 
...
if [ $compile_success -eq 1 ]; then
    printf "\n\033[0;31m----------------------------### double evaluation test FAIL: Could not compile TeX file! ###------------------------------------\033[0m\n"  
    exit 1
fi
if cmp --silent $1/test.xmp_metadata.xml $1/xmp_metadata_expected.xml; then 
    printf "\n\033[0;32m----------------------------### double evaluation test PASS: XMP as expected! ###----------------------------------------\033[0m\n";
else
    printf "\n\033[0;31m----------------------------### double evaluation test FAIL: XMP not as expected! ###------------------------------------\033[0m\n";
    exit 2;
fi